...
You'll use the Hugging Face tokenizer to handle the text-to-token conversion,
...
Tokenization converts text to a list of integers. The GPT-2 tokenizer uses a
...
- [`Tensor.constant(data, dtype, device)`](https://docs.modular.com/max/api/python/tensor#max.tensor.Tensor.constant):
  Creates tensor from Python data

**Device transfer**:

- [`tensor.to(CPU())`](https://docs.modular.com/max/api/python/tensor#max.tensor.Tensor.to):
...
1. Encode the text to token IDs using the tokenizer:
   `token_ids = tokenizer.encode(text, max_length=max_length, truncation=True)`
2. Convert to a MAX tensor with batch dimension:
...
1. Transfer to CPU and convert to NumPy with explicit type annotation:
...
3. Convert to Python list with explicit type annotation:
...
**Next**: In [Step 10](./step_10.md), you'll implement the text generation loop
that uses these functions to produce coherent text autoregressively.
